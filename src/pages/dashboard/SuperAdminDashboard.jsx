...
const PLAN_COLORS = ['#2563EB', '#7C3AED', '#059669', '#D97706', '#DC2626', '#64748B'];
const ROLE_COLORS = ['#2563EB', '#7C3AED', '#059669', '#D97706', '#DC2626', '#0D9488', '#64748B'];
...
function RequestSummary({ request }) {
  ...
  if (request.request_type === 'renewal') {
    return <span>Extend by <strong>{request.extend_days || 365}</strong> days</span>;
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Platform overview — tenants, subscriptions, and system health"
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          deltaType={(stats.suspendedTenants ?? 0) > 0 ? 'up' : 'neutral'}
          icon={AlertTriangle}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          deltaType={(stats.pendingSubscriptionRequests ?? 0) > 0 ? 'up' : 'neutral'}
          icon={Clock}
        />
        <StatCard
          ...
          deltaType={(stats.expiringSubscriptions30Days ?? 0) > 0 ? 'up' : 'neutral'}
          icon={AlertTriangle}
        />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { to: '/tenants', label: 'Organizations', icon: Building2 },
          { to: '/pending-approvals', label: 'Approvals', icon: Clock },
          ...
          <Link
            key={to}
            to={to}
            className="card p-4 flex items-center gap-3 hover:shadow-md hover:border-brand-200 transition-all group"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Tenant Growth — Last 6 Months</h3>
        ...
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Role Distribution — All Companies</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Tenants by Plan</h3>
        ...
        <div className="card p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-red-50 rounded-lg p-3 text-center">
              <p className="text-2xl font-bold text-red-700">{emailDelivery.failed}</p>
              <p className="text-[10px] text-red-600 font-medium uppercase mt-1">Failed</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card">
          <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <h3 className="text-sm font-semibold">Pending Subscription Requests</h3>
            <Link to="/pending-approvals" className="text-xs text-brand-600 hover:underline">Review</Link>
          </div>
        ...
        <div className="card">
          <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <h3 className="text-sm font-semibold">Subscriptions Expiring Soon</h3>
            <Link to="/subscriptions" className="text-xs text-brand-600 hover:underline">Manage</Link>
          </div>
          <ul className="divide-y divide-slate-100">
            {expiringSoon.length === 0 ? (
              ...
              expiringSoon.map((tenant) => (
                <li key={tenant.id} className="px-5 py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs font-medium text-amber-700">
                      {formatDate(tenant.subscription_expiry_date)}
                    </p>
